...
impl Parser {
    ...
    fn parse_array_or_dict(&mut self, start: &Token) -> Expression {
        match self.peek().tok {
            TOK_COLON => {
                self.consume();
                let end = self.close(TOK_RBRACK);
                self.parse_empty_dict(start, &end)
            }
            TOK_RBRACK => {
                let end = self.close(TOK_RBRACK);
                self.parse_empty_array(start, &end)
            }
            _ => {
                let expr = self.parse_expression();
                match self.peek().tok {
    ...
    fn parse_empty_array(&mut self, start: &Token, end: &Token) -> Expression {
    ...
    fn parse_empty_dict(&mut self, start: &Token, end: &Token) -> Expression {
