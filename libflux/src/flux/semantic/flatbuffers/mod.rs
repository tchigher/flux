    ...
    fn done(&mut self, node: Rc<walk::Node<'_>>) {
        ...
        match node {
            ...
            walk::Node::IdentifierExpr(id) => {
                ...
                v.expr_stack.push((ident.as_union_value(), fbsemantic::Expression::IdentifierExpression))
            }
            ...
            walk::Node::MemberExpr(member) => {
                ...
                v.expr_stack.push((mem.as_union_value(), fbsemantic::Expression::MemberExpression)); 
            }
            ...
            walk::Node::OptionStmt(opt) => {
                ...
                v.stmts.push((opt.as_union_value(), fbsemantic::Statement::OptionStatement)); 
            }
...
impl<'a> SerializingVisitorState<'a> {
    ...
    fn pop_expr_with_kind<T>(&mut self, kind: fbsemantic::Expression) -> Option<WIPOffset<T>> {
        match self.expr_stack.pop() {
            ...
            None => {
                self.err = Some(String::from(format!("Tried popping empty expression stack")));
                return None;
            }
        }
    }

    fn pop_ident<T>(&mut self, name: String) -> Option<WIPOffset<T>> {
        match self.identifiers.pop() {
            None => {
                self.err = Some(String::from(format!("Tried popping empty identifier stack: {}", name)));
                return None;
            }
